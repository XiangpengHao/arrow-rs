...
impl Page {
    ...
    pub fn statistics(&self) -> Option<&Statistics> {
        match self {
            Page::DataPage { ref statistics, .. } => statistics.as_ref().map(|x| x.as_ref()),
            Page::DataPageV2 { ref statistics, .. } => statistics.as_ref().map(|x| x.as_ref()),
        ...
        match self.compressed_page {
            Page::DataPage {
                ...
                let data_page_header = crate::format::DataPageHeader {
                    ...
                    statistics: crate::file::statistics::to_thrift(statistics.as_ref().map(|x| x.as_ref())),
                };
