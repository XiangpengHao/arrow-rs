...
impl CachedArrayReader {
    ...
    /// Ensures a batch is cached by fetching it from inner reader if needed
    fn ensure_cached(&mut self, batch_id: usize) -> Result<()> {
        ...
        // Read the batch
        let records_read = self.inner.read_records(self.batch_size())?;
        ...
        self.cache
            .lock()
            .unwrap()
            .insert(self.column_idx, batch_start_row, array);
...
impl ArrayReader for CachedArrayReader {
    ...
    fn read_records(&mut self, batch_size: usize) -> Result<usize> {
        // Add selection for reading these records
        self.selection.push_back(RowSelector::select(batch_size));

        // Determine which batches we need and ensure they're cached
        let start_batch = self.batch_id_from_row(self.current_row);
        let end_batch = self.batch_id_from_row(self.current_row + batch_size - 1);

        for batch_id in start_batch..=end_batch {
            self.ensure_cached(batch_id)?;
        }
